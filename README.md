...
| `produtos` | id, descricao, preco_venda, unidade | — |
| `pedidos` | id, data_emissao, cliente_id, total | → clientes |
| `pedido_itens` | id, pedido_id, produto_id, quantidade, valor_unitario | → pedidos (CASCADE), produtos |
...
```
+---------------------------------------------+
| Cliente: [____________________] [B]         |
| Produto: [____________________] [B]         |
| Un: [__] Vlr: [______] Qtd: [____] Total:[_]|
|                                       [+]   |
+---------------------------------------------+
| [Grid de itens]                             |
|  ID PROD | DESCRICAO | QTD | VLR UNITARIO   |
+---------------------------------------------+
...
```delphi
RS := TResourceStream.Create(HInstance, 'SQLITE3DLL', RT_RCDATA);
...
O `.res` é gerado a partir do `.rc` mais a DLL. Não precisa ir pro repositório Git — basta rodar o `brcc32` uma vez ao clonar o projeto.

### Estrutura de arquivos no computador do cliente

```
C:\SisVendas\
├── sqlite3.dll    ← extraída automaticamente na 1a execução
└── pedidos.db     ← criado automaticamente na 1a execução
```
...
|---|---|
| `sqlite3.dll` (32 ou 64 bits conforme a plataforma alvo) | sqlite.org/download.html → Precompiled Binaries for Windows |
